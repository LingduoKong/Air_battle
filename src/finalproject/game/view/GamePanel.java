...
import edu.uchicago.cs.java.finalproject.game.model.Shield;
...
import java.util.concurrent.CopyOnWriteArrayList;
 ...
 public class GamePanel extends Panel {
	...
	private void drawScore(Graphics g) {
     ...
     private void drawLevel(Graphics g) {
		...
		if (CommandCenter.getLevel() != 0) {
			g.drawString("LEVEL :  " + CommandCenter.getLevel(), nFontWidth, nFontHeight*2);
		}
	}
    ...
    //convert to cartesean points
    for (int nC = 0; nC < nLen; nC++) {
        pntMs[nC] = new Point((int) (10 * dLens[nC] * Math.sin(Math.toRadians(90) + shield.getDegrees()[nC])),
        (int) (10 * dLens[nC] * Math.cos(Math.toRadians(90)
		...
		//for each falcon left (not including the one that is playing)
         g.drawString("SHIELD :  " ,nFontWidth/2, Game.DIM.height - 40);

         for (int nD = 1; nD <= CommandCenter.getFalcon().getShield(); nD++) {
            //create x and y values for the objects to the bottom right using cartesean points again
            for (int nC = 0; nC < shield.getDegrees().length; nC++) {
                nXs[nC] = pntMs[nC].x + (20 * nD) + nFontWidth*2;
                nYs[nC] = pntMs[nC].y + Game.DIM.height - 45;
            }
