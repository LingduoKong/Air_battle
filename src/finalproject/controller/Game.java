	...
	private void checkCollisions() {
		...
		for (Movable movFriend : CommandCenter.movFriends) {
				...
				if (pntFriendCenter.distance(pntFoeCenter) < (nFriendRadiux + nFoeRadiux)) {
					...
					if ((movFriend instanceof Falcon) ){
						if (!CommandCenter.getFalcon().getProtected()){
                            if(!CommandCenter.getFalcon().getbShield()){
            ...
            for (Movable movFloater : CommandCenter.movFloaters) {
				...
				pntFriendCenter = movFriend.getCenter();
				pntFoeCenter = movFloater.getCenter();
				nFriendRadiux = movFriend.getRadius();
				nFoeRadiux = movFloater.getRadius();

				//detect collision
				if (pntFriendCenter.distance(pntFoeCenter) < (nFriendRadiux + nFoeRadiux)) {
                    if (movFloater instanceof MorePower) {
                        CommandCenter.movFloaters.remove(movFloater);
                        CommandCenter.getFalcon().setMorePower(true);
                    }
	...
	private void killFoe(Movable movFoe) {
		...
		if (movFoe instanceof Asteroid){
			...
			if(astExploded.getSize() == 0){
				...
				tupMarkForAdds.add(new Tuple(CommandCenter.movFoes,new Asteroid(1)));
				tupMarkForAdds.add(new Tuple(CommandCenter.movFoes,new Asteroid(1)));
...
//                tupMarkForAdds.add(new Tuple(CommandCenter.movDebris,new DebrisPieces(astExploded)));
            }
	...
	private void spawnNewShipFloater() {
		...
		if (nTick % (SPAWN_NEW_SHIP_FLOATER - nLevel * 7) == 100) {
    ...
    private void spawnNewWeapon() {
        //make the appearance of power-up dependent upon ticks and levels
        //the higher the level the more frequent the appearance
        if (nTick % (SPAWN_NEW_SHIP_FLOATER - nLevel * 17) == 200 ) {
            CommandCenter.movFloaters.add(new MorePower());
        }
    }
    ...
    private void spawnNewShield() {
        //make the appearance of power-up dependent upon ticks and levels
        //the higher the level the more frequent the appearance
//        if (nTick % (SPAWN_NEW_SHIELD - nLevel * 200) == 0) {
//            CommandCenter.movFloaters.add(new Shield());
//        }
        if (nTick % SPAWN_NEW_SHIP_FLOATER - nLevel * 23 == 300) {
            CommandCenter.movFoes.add(new Shield());
        }
    }
	...
	public void keyReleased(KeyEvent e) {
		...
		if (fal != null) {
			...
			case FIRE:
                ...
                if(CommandCenter.getFalcon().getMorePower())
                    CommandCenter.movFriends.add(new TwoLineBullet(fal));
...
//				CommandCenter.movFriends.add(new Cruise(fal));
				//Sound.playSound("laser.wav");
                for (Movable movFoe : CommandCenter.movFoes) {
                    CommandCenter.movFriends.add(new TrackMissiles(fal, movFoe));
                }//end if
				break;
